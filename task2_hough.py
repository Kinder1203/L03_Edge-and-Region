...
def main():
    ...
    hough_threshold = 80            # 동일 직선 위에 존재한다고 '투표(교차)'한 점의 개수가 80개를 넘어설 때만 직선으로 추려냅니다.
    minLineLength = 40              # 아무리 강한 직선이라도 이 픽셀 길이 이하라면 노이즈(먼지)로 간주해 폐기합니다.
    maxLineGap = 5                 # 도중에 끊어짐이 있는 파선이라도 그 간극이 5픽셀 이내라면 연장된 하나의 직선으로 편입시킵니다.
