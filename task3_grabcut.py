...
def main():
    ...
    # 임의 숫자를 하드코딩하지 않고, 이미지 가로폭의 15% 기점(x), 높이의 10% 기점(y) 같이 사진 외곽의 테두리를 배경 샘플로 사용하기 위해 10픽셀 여백을 둡니다.
    x, y = 10, 10 
    # 가로는 영상 너비의 70%만큼 채우고, 세로는 80%를 아우르는 사진 전체를 꽉 채우는 사각형(Rect)을 설정합니다.
    ...
    # 5. 분류가 끝난 마스크 안에는 네 가지 라벨링 정수(cv.GC_BGD:배경, cv.GC_FGD:전경, cv.GC_PR_BGD:배경후보, cv.GC_PR_FGD:전경후보)가 복잡하게 산재해 있습니다. 후처리 정리가 시급합니다.
